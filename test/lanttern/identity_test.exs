defmodule Lanttern.IdentityTest do
  ...
  describe "update_user_email/2" do
    ...
    test "updates the email with a valid token", %{user: user, token: token, email: email} do
      assert {:ok, %{email: ^email}} = Identity.update_user_email(user, token)
    ...
    test "does not update email with invalid token", %{user: user} do
      assert Identity.update_user_email(user, "oops") == {:error, :transaction_aborted}
      assert Repo.get!(User, user.id).email == user.email
      assert Repo.get_by(UserToken, user_id: user.id)
    end

    test "does not update email if user email changed", %{user: user, token: token} do
      assert Identity.update_user_email(%{user | email: "current@example.com"}, token) ==
    ...
    test "does not update email if token expired", %{user: user, token: token} do
      ...
      assert Identity.update_user_email(user, token) == {:error, :transaction_aborted}
      assert Repo.get!(User, user.id).email == user.email
      assert Repo.get_by(UserToken, user_id: user.id)
    end
  end
