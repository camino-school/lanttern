defmodule LantternWeb.AssessmentPointControllerTest do
  ...
  describe "new assessment point" do
    test "renders form", %{conn: conn} do
      conn = get(conn, ~p"/assessments/new")
  ...
  describe "create assessment point" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      conn = post(conn, ~p"/assessments", assessment_point: create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == ~p"/assessments/#{id}"

      conn = get(conn, ~p"/assessments/#{id}")
      assert html_response(conn, 200) =~ "Assessment point #{id}"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/assessments", assessment_point: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Assessment"
    end
  end
  ...
  describe "edit assessment point" do
    ...
    test "renders form for editing chosen assessment", %{
      ...
      conn = get(conn, ~p"/assessments/#{assessment_point}/edit")
  ...
  describe "update assessment point" do
    ...
    test "redirects when data is valid", %{conn: conn, assessment_point: assessment_point} do
      conn = put(conn, ~p"/assessments/#{assessment_point}", assessment_point: @update_attrs)
      assert redirected_to(conn) == ~p"/assessments/#{assessment_point}"

      conn = get(conn, ~p"/assessments/#{assessment_point}")
      assert html_response(conn, 200) =~ "some updated name"
    end

    test "renders errors when data is invalid", %{conn: conn, assessment_point: assessment_point} do
      conn = put(conn, ~p"/assessments/#{assessment_point}", assessment_point: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Assessment"
    end
  end
  ...
  describe "delete assessment point" do
    ...
    test "deletes chosen assessment point", %{conn: conn, assessment_point: assessment_point} do
      conn = delete(conn, ~p"/assessments/#{assessment_point}")
      assert redirected_to(conn) == ~p"/assessments"

      assert_error_sent 404, fn ->
        get(conn, ~p"/assessments/#{assessment_point}")
      end
    end
  end
