defmodule LantternWeb.AssessmentPointControllerTest do
  ...
  describe "index" do
    test "lists all assessment points", %{conn: conn} do
      conn = get(conn, ~p"/admin/assessment_points")
  ...
  describe "new assessment point" do
    test "renders form", %{conn: conn} do
      conn = get(conn, ~p"/admin/assessment_points/new")
  ...
  describe "create assessment point" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      conn = post(conn, ~p"/admin/assessment_points", assessment_point: create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == ~p"/admin/assessment_points/#{id}"

      conn = get(conn, ~p"/admin/assessment_points/#{id}")
      assert html_response(conn, 200) =~ "Assessment point #{id}"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn =
        post(conn, ~p"/admin/assessment_points", assessment_point: @invalid_attrs)
  ...
  describe "edit assessment point" do
    ...
    test "renders form for editing chosen assessment", %{
      ...
      conn = get(conn, ~p"/admin/assessment_points/#{assessment_point}/edit")
  ...
  describe "update assessment point" do
    ...
    test "redirects when data is valid", %{conn: conn, assessment_point: assessment_point} do
      conn =
        put(conn, ~p"/admin/assessment_points/#{assessment_point}",
          assessment_point: @update_attrs
        )

      assert redirected_to(conn) == ~p"/admin/assessment_points/#{assessment_point}"

      conn = get(conn, ~p"/admin/assessment_points/#{assessment_point}")
  ...
  describe "delete assessment point" do
    ...
    test "deletes chosen assessment point", %{conn: conn, assessment_point: assessment_point} do
      conn = delete(conn, ~p"/admin/assessment_points/#{assessment_point}")
      assert redirected_to(conn) == ~p"/admin/assessment_points"

      assert_error_sent 404, fn ->
        get(conn, ~p"/admin/assessment_points/#{assessment_point}")
      end
    end
  end
