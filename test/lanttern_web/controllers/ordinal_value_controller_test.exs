defmodule LantternWeb.OrdinalValueControllerTest do
  ...
  describe "index" do
    test "lists all ordinal_values", %{conn: conn} do
      conn = get(conn, ~p"/admin/ordinal_values")
  ...
  describe "new ordinal_value" do
    test "renders form", %{conn: conn} do
      conn = get(conn, ~p"/admin/ordinal_values/new")
  ...
  describe "create ordinal_value" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      conn = post(conn, ~p"/admin/ordinal_values", ordinal_value: create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == ~p"/admin/ordinal_values/#{id}"

      conn = get(conn, ~p"/admin/ordinal_values/#{id}")
      assert html_response(conn, 200) =~ "Ordinal value #{id}"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/admin/ordinal_values", ordinal_value: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Ordinal value"
    end
  end
  ...
  describe "edit ordinal_value" do
    ...
    test "renders form for editing chosen ordinal_value", %{
      ...
      conn = get(conn, ~p"/admin/ordinal_values/#{ordinal_value}/edit")
  ...
  describe "update ordinal_value" do
    ...
    test "redirects when data is valid", %{conn: conn, ordinal_value: ordinal_value} do
      conn =
        put(conn, ~p"/admin/ordinal_values/#{ordinal_value}", ordinal_value: @update_attrs)

      assert redirected_to(conn) == ~p"/admin/ordinal_values/#{ordinal_value}"

      conn = get(conn, ~p"/admin/ordinal_values/#{ordinal_value}")
      assert html_response(conn, 200) =~ "some updated name"
    end

    test "renders errors when data is invalid", %{conn: conn, ordinal_value: ordinal_value} do
      conn =
        put(conn, ~p"/admin/ordinal_values/#{ordinal_value}", ordinal_value: @invalid_attrs)
  ...
  describe "delete ordinal_value" do
    ...
    test "deletes chosen ordinal_value", %{conn: conn, ordinal_value: ordinal_value} do
      conn = delete(conn, ~p"/admin/ordinal_values/#{ordinal_value}")
      assert redirected_to(conn) == ~p"/admin/ordinal_values"

      assert_error_sent 404, fn ->
        get(conn, ~p"/admin/ordinal_values/#{ordinal_value}")
      end
    end
  end
