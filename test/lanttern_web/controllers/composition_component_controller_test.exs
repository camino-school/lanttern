defmodule LantternWeb.CompositionComponentControllerTest do
  ...
  describe "index" do
    test "lists all grade_composition_components", %{conn: conn} do
      conn = get(conn, ~p"/grade_composition_components")
  ...
  describe "new composition_component" do
    test "renders form", %{conn: conn} do
      conn = get(conn, ~p"/grade_composition_components/new")
  ...
  describe "create composition_component" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      conn = post(conn, ~p"/grade_composition_components", composition_component: create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == ~p"/grade_composition_components/#{id}"

      conn = get(conn, ~p"/grade_composition_components/#{id}")
      assert html_response(conn, 200) =~ "Composition component #{id}"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/grade_composition_components", composition_component: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Composition component"
    end
  end
  ...
  describe "edit composition_component" do
    ...
    test "renders form for editing chosen composition_component", %{
      ...
      conn = get(conn, ~p"/grade_composition_components/#{composition_component}/edit")
  ...
  describe "update composition_component" do
    ...
    test "redirects when data is valid", %{
      ...
      conn =
        put(conn, ~p"/grade_composition_components/#{composition_component}",
          composition_component: @update_attrs
        )

      assert redirected_to(conn) == ~p"/grade_composition_components/#{composition_component}"

      conn = get(conn, ~p"/grade_composition_components/#{composition_component}")
  ...
  describe "delete composition_component" do
    ...
    test "deletes chosen composition_component", %{
      ...
      conn = delete(conn, ~p"/grade_composition_components/#{composition_component}")
      assert redirected_to(conn) == ~p"/grade_composition_components"

      assert_error_sent 404, fn ->
        get(conn, ~p"/grade_composition_components/#{composition_component}")
      end
    end
  end
