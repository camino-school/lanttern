defmodule LantternWeb.StrandsLibraryLiveTest do
  use LantternWeb.ConnCase

  alias Lanttern.LearningContextFixtures
  alias Lanttern.TaxonomyFixtures

  @live_view_path "/strands/library"

  setup [:register_and_log_in_teacher]

  describe "Strands library live view basic navigation" do
    test "disconnected and connected mount", %{conn: conn} do
      conn = get(conn, @live_view_path)

      assert html_response(conn, 200) =~ ~r"<h1 .+>\s*Library\s*<\/h1>"
    ...
    test "list strands and navigate to detail", %{conn: conn} do
      ...
      assert_redirect(view, "/strands/#{strand.id}")
    end
  end
  ...
  describe "Strand management" do
    ...
    test "create strand", %{conn: conn} do
      ...
      view |> element("a", "Create strand") |> render_click()
      assert_patch(view, "/strands/library/new")
