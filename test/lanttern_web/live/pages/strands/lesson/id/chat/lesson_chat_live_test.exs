defmodule LantternWeb.LessonChatLiveTest do
  ...
  defp create_conversation_with_message(profile, lesson) do
    conversation =
      insert(:conversation, %{
        profile: profile,
  ...
  describe "Access control" do
    test "redirects to dashboard without agents_management permission", %{conn: conn} do
      ...
      conn
      |> visit("#{@live_view_base_path}/#{lesson.id}/chat")
  ...
  describe "Rename conversation" do
    ...
    test "rename button is visible when there's a conversation selected", %{
      ...
      conversation = create_conversation_with_message(profile, lesson)

      # Update the name for this specific test
      Repo.update!(Ecto.Changeset.change(conversation, name: "Test Conversation"))

      conn
      |> visit("#{@live_view_base_path}/#{lesson.id}/chat/#{conversation.id}")
