defmodule LantternWeb.FormComponents do
  ...
  def textarea(assigns) do
    ...
    <textarea
      ...
      name={@name}
      class={[
        "block w-full min-h-80 rounded-xs border-0 shadow-xs ring-1 sm:text-sm sm:leading-6",
  ...
  def base_input(%{type: "color"} = assigns) do
    assigns = assign(assigns, :tailwind_colors, @tailwind_colors)

    ~H"""
    <div id={"#{@id}-color-picker"} class="relative" phx-hook=".ColorPicker">
      <div class="flex items-center gap-2">
        <div
          class="shrink-0 w-9 h-9 rounded-xs border border-ltrn-lighter"
      ...
      <div
        id={"#{@id}-palette"}
        class="hidden absolute z-20 mt-1 p-2 max-h-64 overflow-y-auto rounded-xs shadow-lg ring-1 ring-ltrn-lighter bg-white"
      >
        <div class="flex gap-px"></div>
        <div :for={{_family, shades} <- @tailwind_colors} class="flex gap-px mt-px">
          <button
            :for={{_shade, hex} <- shades}
            type="button"
            tabindex="-1"
            data-color={hex}
            class="w-5 h-5 rounded-xs hover:scale-125 hover:z-10 transition-transform"
    ...
    <script :type={Phoenix.LiveView.ColocatedHook} name=".ColorPicker">
      export default {
        mounted() {
          ...
          this.palette = this.el.querySelector("[id$='-palette']");

          this.input.addEventListener("focus", () => this.openPalette());

          this.palette.addEventListener("click", (e) => {
            const btn = e.target.closest("[data-color]");
            if (!btn) return;
            this.input.value = btn.dataset.color;
            this.input.dispatchEvent(new Event("input", { bubbles: true }));
            this.closePalette();
          });

          this._onKeydown = (e) => {
            if (e.key === "Tab") this.closePalette();
          };

          this._onClickAway = (e) => {
            if (!this.el.contains(e.target)) this.closePalette();
          };
