defmodule LantternWeb.Rubrics.RubricFormOverlayComponent do
    ...
    <div phx-remove={JS.exec("phx-remove", to: "##{@id}")}>
      <.slide_over id={@id} show={true} on_cancel={@on_cancel}>
        ...
        >
          ...
          <p class="mb-6"><%= gettext("Link rubric to assessment points") %></p>
  ...
  defp descriptors_fields(%{scale: nil} = assigns) do
    ~H"""
    <p class="mb-6 text-ltrn-subtle"><%= gettext("Select a scale to create descriptors") %></p>
    """
  end
  ...
  defp descriptors_fields(%{scale: %{type: "ordinal"}} = assigns) do
    ...
    <.inputs_for :let={ef} field={@field}>
      ...
      <.input type="textarea" field={ef[:descriptor]} class="mb-6" phx-debounce="1500">
        <:custom_label>
          <.live_component
  ...
  defp assessment_point_option(assigns) do
    {assessment_point_name, badge_text, badge_theme} =
      case assigns.assessment_point do
        %{moment: %Moment{} = moment} -> {assigns.assessment_point.name, moment.name, "default"}
        _ -> {gettext("Goal final assessment"), gettext("Goal"), "dark"}
      end
    ...
    ~H"""
    <.card_base class="flex items-center gap-6 p-6 mb-6">
      <button
        type="button"
  ...
  defp assign_assessment_points(socket) do
    ...
    |> assign(:selected_assessment_points_ids, selected_assessment_points_ids)
  end
