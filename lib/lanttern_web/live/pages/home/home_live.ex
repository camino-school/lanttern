defmodule LantternWeb.HomeLive do
  ...
  def handle_event("card_lookout", %{"id" => id}, socket) do
    ...
    socket =
      socket
      |> push_patch(to: "#{base_path}?message=#{id}")
  ...
  def render(assigns) do
    ...
    <Layouts.app_logged_in flash={@flash} current_user={@current_user} current_path={@current_path}>
      ...
      <.responsive_container class="mt-16">
        <.live_component
          module={LantternWeb.MessageBoard.MessageBoardGridComponentV2}
    ...
    <.live_component
      ...
      on_cancel={
        JS.patch(if @profile_type == "guardian", do: ~p"/guardian_v2", else: ~p"/student_v2")
      }
      sticky_header={true}
      full_w={true}
      base_path={if @profile_type == "guardian", do: ~p"/guardian_v2", else: ~p"/student_v2"}
