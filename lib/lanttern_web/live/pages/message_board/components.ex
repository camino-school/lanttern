defmodule LantternWeb.MessageBoard.Components do
  ...
  def message_card_admin(assigns) do
    ...
    <div
      ...
      class="flex flex-col overflow-hidden w-full h-full aspect-square relative bg-white/90 backdrop-blur-sm rounded-md border border-l-12 border-ltrn-lightest shadow-xl z-20"
      style={"
    ...
    >
      <%= if @message.cover && @message.cover != "" && !@message.archived_at do %>
        <div class="w-full h-24 overflow-hidden rounded-tr-md">
          <img
        ...
        <div class="flex items-start gap-4">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-2">
              <h3
                class="font-display font-black text-lg line-clamp-2 break-words"
                title={@message.name}
              >
                {@message.name}
              </h3>
  ...
  def message_card(assigns) do
    ...
    <div
      class={[
        "flex flex-col overflow-hidden w-[300px] h-[300px] bg-white/90 backdrop-blur-sm rounded-sm border border-l-12 border-ltrn-lightest shadow-xl z-20 relative shrink-0",
        @class
      ]}
      style={"border-color: var(--color-ltrn-lightest); border-left-color: #{@message.color};"}
    >
      <%= if @message.cover && @message.cover != "" do %>
        <div class="w-full h-32 overflow-hidden rounded-tr-md">
          <img
            src={@message.cover || "/placeholder.svg"}
            alt={@message.name}
