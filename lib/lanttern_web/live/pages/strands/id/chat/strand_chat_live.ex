defmodule LantternWeb.StrandChatLive do
  ...
  defp assign_strand(socket, %{"strand_id" => id}) do
    ...
    |> case do
      ...
      strand ->
        ...
        |> assign(:page_title, "#{gettext("Chat")} | #{strand.name}")
    end
  end
  ...
  defp assign_conversations(socket) do
    conversations =
      AgentChat.list_conversations(
        ...
        # strand lessons chats should be used in the future,
        # but for now let's focus on the strand only conversations
        lesson_id: nil
      )
  ...
  defp apply_action(socket, :show, %{"conversation_id" => id}) do
    case AgentChat.get_conversation(socket.assigns.current_scope, id) do
      nil ->
        ...
        |> push_navigate(to: ~p"/strands/#{socket.assigns.strand}/chat")
  ...
  def handle_info({ConversationComponent, {:conversation_created, conversation}}, socket) do
    socket =
      push_patch(
        socket,
        to: ~p"/strands/#{socket.assigns.strand}/chat/#{conversation.id}"
      )
