defmodule Lanttern.MessageBoard.MessageV2 do
  ...
  def changeset(message, attrs) do
    changeset =
      message
      |> cast(attrs, [:name, :description, :school_id, :send_to, :section_id, :subtitle, :color, :cover, :position])
      |> validate_required([:name, :description, :school_id, :send_to, :section_id])
      |> check_constraint(:send_to, name: :valid_send_to, message: gettext("Send to must be 'school' or 'classes'"))
  ...
  defp cast_classes(changeset, classes_ids) when is_list(classes_ids) do
    ...
    changeset |> put_change(:message_classes, message_classes_params) |> cast_assoc(:message_classes)
  end
